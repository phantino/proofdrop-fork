...
import ApiKeysInfo from "@/components/api-keys-info";
...
export default function WalletConnection() {
  ...
  const handleMintScore = async () => {
    ...
    // First time: analyze and show score
    if (!showMintButton) {
      setShowMintButton(true);
      return;
    }
    
    // Second time: mint NFT
    try {
            ...
            <div className="gradient-border" data-testid="score-display">
              <div className="gradient-border-content">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  {analysis.score.canMint && showMintButton ? (
                    <Button 
                    ...
                    <Button 
                      onClick={() => setShowMintButton(true)}
                      disabled={!analysis.score.canMint}
                      className={`flex-1 bg-gradient-to-r ${
                        analysis.score.canMint 
                          ? 'from-electric-purple to-bright-purple hover:from-bright-purple hover:to-electric-purple' 
                          : 'from-gray-600 to-gray-700 cursor-not-allowed'
                      } py-3 rounded-lg font-semibold transition-all duration-300`}
                      data-testid="show-mint-button"
                    >
                      {analysis.score.canMint 
                        ? `✅ Eligible to Mint (Score: ${analysis.score.total})` 
                        : `❌ Score too low (${analysis.score.total}/5 needed)`
                      }
                    </Button>
                  )}
                  <Button 
